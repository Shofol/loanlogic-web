                ...
                .map((debt) => {
                  return (
                    <tr key={debt.id}>
                      ...
                      <td>{debt.status == "PAID" ? "PAGADO" : (debt.status == "PENDING" ? "PENDIENTE" : (debt.status == "PARTIALLY_PAID" ? "PAGO PARCIAL" : (debt.status == "UNPAID" ? "IMPAGO": (debt.status == "CANCELED" ? "CANCELADO" : debt.status ))))}</td>
